...
namespace Web_Api_OES.Controllers
{
    ...
    public class AuthController : ApiController
    {
        ...
        public IHttpActionResult ForgotPassword([FromBody] ForgotPasswordRequest req)
        {
            ...
            // 🚨 New validation: Only students can reset password
        ...
        [HttpPost]
        [Route("verifyotp")]
        public IHttpActionResult VerifyOtp([FromBody] VerifyOtpRequest req)
        {
            var normalizedEmail = req.Email.Trim().ToLower();
            var user = db.Users.FirstOrDefault(u => u.email.Trim().ToLower() == normalizedEmail);
        ...
        public IHttpActionResult ResetPassword([FromBody] ResetPasswordRequest req)
        {
            ...
            if (user.role != null && user.role.ToLower() != "student")
            {
                return BadRequest("Admins cannot reset password");
            }

            user.password = req.NewPassword;   // consider hashing later
            db.SaveChanges();
        ...
        private void SendEmail(string to, string subject, string body)
        {
            ...
            var password = "punt gpsv ogqm mzjd"; // your Gmail App Password
